...
// ── Mock stats (replace with Spring Boot API calls) ──────────────────────────
const TENANT_STATS = [
  {
    label: "Total Revenue",
    value: "$12,480",
...
export default function AdminDashboard() {
  ...
  const stats = isSuperAdmin ? SUPER_STATS : TENANT_STATS;
  ...
  return (
    <div className="space-y-8 max-w-screen-xl">
      ...
      <div>
        ...
        <p className="text-sm text-slate-400 mt-1">
          {isSuperAdmin
            ...
            : `Store overview · ${user.tenantName}`}
        </p>
      </div>
              ...
              <thead>
                <tr className="border-b border-slate-50">
                  {["Order", "Customer", "Product", "Total", "Status"].map((h) => (
                    <th key={h} className="text-left text-xs font-semibold text-slate-400 px-6 py-3">
                      {h}
                    </th>
                  ))}
                </tr>
      ...
      {/* Architecture note card — explains SAAS model visually */}
      {isSuperAdmin && (
        <div className="bg-slate-800 rounded-2xl p-6 text-white">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h3 className="text-sm font-bold mb-1">Multi-Tenant Architecture</h3>
              <p className="text-xs text-slate-300 leading-relaxed max-w-2xl">
                Each flower company is a <strong className="text-emerald-400">Tenant</strong> with an isolated
                view of their products, orders, and customers. The Spring Boot backend enforces
                <strong className="text-emerald-400"> tenant_id</strong> on every database query — tenants
                cannot access each other's data. You (Smagyo) see everything from this super-admin view.
              </p>
              <div className="flex flex-wrap gap-2 mt-3">
                {["tenant_id on every table", "JWT contains tenant_id", "Spring Security TenantFilter", "Shared schema, isolated data"].map((t) => (
                  <span key={t} className="text-[10px] font-medium px-2.5 py-1 rounded-full bg-white/10 text-slate-300">
                    {t}
                  </span>
                ))}
              </div>
